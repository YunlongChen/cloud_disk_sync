...
use crate::providers::{DownloadResult, StorageProvider, UploadResult, FileInfo};
            ...
            Some(token_value) => {
                Ok(Self {
                    ...
                    refresh_token: config.credentials.get("refresh_token")
                        .cloned()
                        .unwrap_or_default(),
                    rate_limiter: Arc::new(SlidingWindowRateLimiter { window_size: Duration::from_secs(1), max_requests: 1u64, requests: Mutex::new(vec![]) }),
                })
            }
        }
    }
...
impl StorageProvider for AliYunDriveProvider {
    async fn list(&self, path: &str) -> Result<Vec<FileInfo>, SyncError> {
        Ok(vec![FileInfo { path: path.to_string(), size: 0, modified: 0, hash: None, is_dir: true }])
    }
    ...
    async fn stat(&self, path: &str) -> Result<FileInfo, SyncError> {
        Ok(FileInfo { path: path.to_string(), size: 0, modified: 0, hash: None, is_dir: true })
    }
