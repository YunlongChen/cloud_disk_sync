...
use crate::providers::{StorageProvider, FileInfo};
...
pub async fn run_browse_tui(provider: Arc<dyn StorageProvider>, initial_path: String) -> Result<(), Box<dyn std::error::Error>> {
    // Setup terminal
    enable_raw_mode()?;
    ...
    let start_path = if initial_path.starts_with('/') { initial_path } else { format!("/{}", initial_path) };
...
async fn run_app(terminal: &mut Terminal<CrosstermBackend<io::Stdout>>, app: &mut AppState, provider: Arc<dyn StorageProvider>) -> io::Result<()> {
    loop {
    ...
    if app.loading && app.files.is_empty() {
        ...
        let rows = app.files.iter().enumerate().map(|(i, file)| {
            ...
            let name = if file.path == "/" {
                ...
                file.path.trim_end_matches('/').split('/').last().unwrap_or(&file.path)
            };
            ...
            let metadata_color = if is_selected { Color::White } else { Color::DarkGray };
        ...
        .block(Block::default().borders(Borders::ALL).border_type(BorderType::Rounded).title(" Files "))
